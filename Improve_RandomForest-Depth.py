...
# 在微博传播深度下 改进的随机森林算法 预测50条数据误差 0.05208612440191387
# 读取数据
...
for leaf_size in sample_leaf_options:
    for n_estimators_size in n_estimators_options:
        ...
        # 训练模型
        alg.fit(source[x_col][:9737], source['depth9'][:9737])
        # 预测微博测试数据的转发次数
        predict = alg.predict(source[x_col][9737:])
        # 计算真实值与预测值之差
        # 使用微博元数据的后50位真实数据与预测出来的50条数据来进行对比
        # 在(n,n)的模型下，分别比较50位数据的误差
        for i in range(len(predict)):
            aa = abs(source['depth9'][9737 + i] - predict[i])
            # 如果真实值与预测值之差为零则准确率为100%
            if aa == 0:
                bb = 1
            # 如不为0则计算误差
            else:
                bb = 1 - float(aa) / float(source['depth9'][9737 + i])
...
plt.legend()
plt.savefig('ImRF-depth-50.png', dpi=400, bbox_inches='tight')
plt.show()
